...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $origen = $_POST['origen'] ?? 'jerseys'; // Valor por defecto
    ...
    if (!$cod_producto || !$nombre || !$modelo || !$descripcion || !$rutaDestino) {
        $_SESSION['error'] = "Alguno de los campos introducidos no son correctos.";
        header('Location: ../'.$origen.'.php');
        exit();
    } else {
        $insertarProducto = $conexionBaseDatos->prepare("INSERT INTO Productos (cod_producto, nombre, modelo, descripcion, CIF,imagen) VALUES (?, ?, ?, ?, ?, ?)");
        $insertarProducto->bind_param("ssisss", $cod_producto, $nombre, $modelo, $descripcion, $cif, $rutaDestino);
